...
**Consequences:** R1 (handler split) still before R3. PR 1.2 / Phase 2 vocabulary cache precede or ship with executor patch path. MCP adapter, if ever built, wraps the same tool registry — not a second orchestrator. **Write-through for MVP** (Session 3b): `apply_patch` writes RXData directly to the opened Essentials project — no staging overlay until GUI G3; amend this ADR when staging lands. See Session 3 snapshot in [`design-process.md`](../design-process.md).
